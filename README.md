...
это нормально, не ошибка. Бесплатный Postgres (см. ниже) удаляется через
30 дней после создания — тоже ограничение Render, не бага в проекте.

## Пароль и общая база субподрядчиков

- **Пароль на сайт.** При создании Blueprint'а Render спросит значение
  переменной `APP_PASSWORD` (в дашборде: сервис `subcontractor-platform-api` →
  Environment) — один общий пароль на всю команду. Бэкенд проверяет его на
  каждый запрос к `/api/*` (см. `backend/main.py`), так что защищены и данные,
  а не только сама страница. Поменять пароль позже — просто отредактировать
  значение переменной в дашборде, без передеплоя кода.
- **Общая база.** `render.yaml` создаёт бесплатный Render Postgres
  (`subcontractor-platform-db`) и подключает его к бэкенду. Excel-файл базы
  субподрядчиков, загруженный любым человеком, сохраняется туда и виден всем,
  кто открывает сайт (раньше — только в `localStorage` в браузере того, кто
  загрузил).
  **Ограничение бесплатного плана:** база удаляется через 30 дней после
  создания. Если данные должны сохраниться дольше — до истечения 30 дней
  поменяйте в `render.yaml` (или прямо в дашборде) `plan: free` на платный,
  например `basic-256mb` (от $6/мес), либо периодически выгружайте базу
  (кнопка экспорта категории в `.xlsx` уже есть в интерфейсе).
